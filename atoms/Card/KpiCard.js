...
const KpiCard = (props) => {
  ...
  if (isLoading) {
    return (
      <div className="rounded-lg w-auto h-25 p-2 backdrop-blur-[20px]">
        <Skeleton active paragraph={{ rows: 2, width: "100%" }} />
      </div>
    );
  }
  return (
    <div className="font-poppins rounded-lg w-auto h-fit p-5 backdrop-blur-[50px] bg-black/60 border border-gray-500/20">
      <h4 className="text-sm font-semibold">{name}</h4>
      <h2 className="mb-1.5 mt-1.5 text-2xl font-bold">{`${prefix} ${value} ${postfix}`}</h2>
      {percentChange && (
